...
struct redundant_logging_tx_event
{
    /** The memory region registered first. */
    struct pmem_st original;
...
static struct transaction_ops
{
    ...
    struct redundant_logging_tx_event **redundant_logging_tx_evs;

    UWord redundant_logging_tx_reg;
...
void init_transactions(Bool transactions_only, Bool tree_reorganization, Bool redundant_logging, Bool print_bug_detail)
{
    ...
    trans.oot_stores = VG_(malloc)("pmc.trans.cpci.3", MAX_OOT_STORES * sizeof(struct pmem_st *));

    trans.excludes = VG_(OSetGen_Create)(/*keyOff*/ 0, cmp_pmem_st,
                                         VG_(malloc), "pmc.trans.cpci.4", VG_(free));

    trans.cross_tx_evs = VG_(malloc)("pmc.trans.cpci.5", MAX_CROSS_EVS * sizeof(struct transaction_ops *));

    trans.redundant_logging_tx_evs = VG_(malloc)("pmc.trans.cpci.5", MAX_CROSS_EVS * sizeof(struct redundant_logging_tx_event *));
    trans.verbose = (VG_(clo_verbosity) > 1);

    trans.transactions_only = transactions_only;
    trans.tree_reorganization = tree_reorganization;
    trans.redundant_logging = redundant_logging;
    trans.print_bug_detail = print_bug_detail;
}
...
print_regions(UWord tx_id)
{
    ...
    while ((tmp = VG_(OSetGen_Next)(tx->regions)) != NULL)
    {
        ...
        if (trans.tree_reorganization || trans.print_bug_detail)
        {
            VG_(pp_ExeContext)
            (tmp->context);
        }
...
print_cross_evs(void)
{
    if (trans.cross_tx_reg)
    {
        ...
        ("\n");
        VG_(umsg)
        ("Number of overlapping regions registered in different "
         "transactions: %lu\n",
         trans.cross_tx_reg);
        VG_(umsg)
        ("Overlapping regions:\n");
        ...
        for (i = 0; i < trans.cross_tx_reg; ++i)
        {
            ...
            {
                VG_(pp_ExeContext)
                (tmp->duplicate.context);
            }
            VG_(umsg)
            ("\tAddress: 0x%lx\tsize: %llu\ttx_id: %lu\n",
...
static void
print_redundant_logging_evs(void)
{
    if (trans.redundant_logging_tx_reg)
    {
        VG_(umsg)
        ("\n");
        VG_(umsg)
        ("Number of overlapping regions registered in a same "
         "transactions: %lu\n",
         trans.redundant_logging_tx_reg);
        VG_(umsg)
        ("Overlapping regions:\n");
        struct redundant_logging_tx_event *tmp = NULL;
        Int i;
        for (i = 0; i < trans.redundant_logging_tx_reg; ++i)
        {
            tmp = trans.redundant_logging_tx_evs[i];
            VG_(umsg)
            ("[%d] ", i);
            if (trans.tree_reorganization || trans.print_bug_detail)
            {
            ...
            VG_(umsg)
            ("\tAddress: 0x%lx\tsize: %llu\ttx_id: %lu\n",
             tmp->duplicate.addr, tmp->duplicate.size, tmp->tx_id);
            VG_(umsg)
            ("   First registered here:\n[%d]'", i);
            if (trans.tree_reorganization || trans.print_bug_detail)
            {
            ...
            VG_(umsg)
            ("\tAddress: 0x%lx\tsize: %llu\ttx_id: %lu\n",
             tmp->original.addr, tmp->original.size, tmp->tx_id);
        }
    }
}
...
print_cross_error(void)
{
    ...
    VG_(umsg)
    ("This means your program is tracking the same memory regions"
...
static void
print_redundant_logging_error(void)
{
    VG_(umsg)
    ("Number of overlapping regions registered in a same "
     "transaction exceeded %lu\n\n",
     MAX_CROSS_EVS);

    VG_(umsg)
    ("This means your program is tracking the same memory regions"
     " within a same transactions. This is a potential performance"
     " issue.\n");
    VG_(message_flush)
    ();

    print_redundant_logging_evs();
}
...
static void
register_redundant_logging_event(const struct pmem_st *orig,
                                 const struct pmem_st *dup, UWord tx_id)
{
    if (UNLIKELY(trans.redundant_logging_tx_reg == MAX_CROSS_EVS))
    {
        print_redundant_logging_error();
        VG_(exit)
        (-1);
    }

    struct redundant_logging_tx_event *to_insert = VG_(malloc)("pmc.trans.cpci.5",
                                                               sizeof(struct redundant_logging_tx_event));
    to_insert->original = *orig;
    to_insert->duplicate = *dup;
    to_insert->tx_id = tx_id;

    trans.redundant_logging_tx_evs[(trans.redundant_logging_tx_reg)++] = to_insert;
}
...
{
    return ((store->addr == tx->cached_region.addr) && (store->size == tx->cached_region.size));
}
...
void register_new_tx(UWord tx_id)
{
    ...
    if (trans.verbose)
        VG_(dmsg)
        ("Starting transaction: %lu, nesting %lu\n", tx_id,
         txinf->counter);
...
UInt remove_tx(UWord tx_id)
{
    ...
    (tx->regions);
    VG_(OSetGen_Remove)
    (trans.transactions, &tx_id);
    VG_(OSetGen_FreeNode)
    (trans.transactions, tx);
...
is_tx_in_thread(UWord tx_id)
{
    ...
    if (tinfo == NULL)
    {
        if (trans.verbose)
            VG_(dmsg)
            ("thread %lu not part of any transaction\n", thread_id);
        return False;
    }
...
UInt add_obj_to_tx(UWord tx_id, UWord base, UWord size)
{
    ...
    while ((tx_iter = VG_(OSetGen_Next)(trans.transactions)) != NULL)
    {
        ...
        {
            if (trans.redundant_logging)
            {
                if (cmp_pmem_st(&reg, &(tx_iter->cached_region)) == 0)
                    register_redundant_logging_event(&tx_iter->cached_region,
                                                     &reg, tx_id);
                else if (is_in_mapping_set(&reg, tx_iter->regions))
                    register_redundant_logging_event(VG_(OSetGen_Lookup)(tx_iter->regions, &reg),
                                                     &reg, tx_id);
                continue;
            }
            else
    ...
    if (LIKELY((tx->cached_region.addr != 0) && (tx->cached_region.size != 0)))
    {
...
void print_tx_summary(void)
{
    ...
    if (active_txs != 0)
    {
        ...
        ("\n");
        VG_(umsg)
        ("Number of active transactions: %lu\n", active_txs);
        VG_(OSetGen_ResetIter)
        (trans.transactions);
        struct tx_info *tmp_tx;
        UWord idx = 0;
...
{
    UWord active_txs = VG_(OSetGen_Size)(trans.transactions);
    return trans.oot_stores_reg + trans.cross_tx_reg + active_txs + trans.redundant_logging_tx_reg;
}
...
{
    VG_(umsg)
    ("The number of out of transaction stores exceeded %lu\n\n",
     limit);

    VG_(umsg)
    ("This means your applications is changing objects that are not"
