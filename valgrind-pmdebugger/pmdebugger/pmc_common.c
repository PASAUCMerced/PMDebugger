...
{
    const struct pmem_st_order *lhs = (const struct pmem_st_order *)(key);
    const struct pmem_st_order *rhs = (const struct pmem_st_order *)(elem);
...
UWord check_overlap(const struct pmem_st *lhs, const struct pmem_st *rhs)
{
    if (cmp_pmem_st(lhs, rhs))
        ...
        return 0;
    else if ((lhs->addr < rhs->addr) || (lhs->addr + lhs->size) > (rhs->addr + rhs->size))
...
void add_region(const struct pmem_st *region, OSet *region_set)
{
    ...
    *entry = *region;
    ...
    search_entry.addr -= 1;
    search_entry.size += 2;
    while ((old_entry = VG_(OSetGen_Lookup)(region_set, &search_entry)) != NULL)
    {
...
void remove_region(const struct pmem_st *region, OSet *region_set)
{
    ...
    (region_set);
    struct pmem_st tmp;
    while ((modified_entry = VG_(OSetGen_Next)(region_set)) !=
           NULL)
    {
        if (cmp_pmem_st(modified_entry, region) != 0)
            continue;
