...
handle_with_mult_stores(struct pmem_st *store)
{
    ...
    NULL) {
        ...
        //老的和新的store操作是一样的，不做处理（一个连续的写，不被看作是persistent memory问题
...
*/
//IRSB由不同的statment组成，这个函数整体上的过程是：把原本的IRSB （bb）添加自己想要加入的函数功能，然后加上原本的的内容，然后生成新的IRSB （sbout），将sbout返回。
static IRSB*
        ...
        IRType gWordTy, IRType hWordTy)
{
    ...
    /* Set up SB */
    sbOut = deepCopyIRSBExceptStmts(bb);//拷贝了bb到sbout
    ...
    for (/*use current i*/; i < bb->stmts_used; i++) {
        ...
        switch (st->tag) {
            ...
            //因为类似于fence指令了，所以清空所有前面已经标记flushed的内存，减少重复检查？
...
pmc_post_clo_init(void)
{
    pmem.pmem_stores = VG_(OSetGen_Create)(/*keyOff*/0, cmp_pmem_st,
            VG_(malloc), "pmc.main.cpci.1", VG_(free));//创建一个OSet数据结构来存储Pmem_store
    ...
    init_transactions(pmem.transactions_only);//初始化transaction
