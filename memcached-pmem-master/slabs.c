...
void slabs_init(const size_t limit, const double factor, const bool prealloc, const uint32_t *slab_sizes) {
    ...
    while (++i < MAX_NUMBER_OF_SLAB_CLASSES-POWER_SMALLEST) {
        if (slab_sizes != NULL) {
            if (slab_sizes[i-POWER_SMALLEST] == 0)
                break;
            size = slab_sizes[i-POWER_SMALLEST];
...
#ifdef PSLAB
#define get_page_from_global_pool() \
...
static int (*newslabs_funcs[])(const unsigned int) = {
    do_slabs_newslab_from_dram,
    do_slabs_newslab_from_pmem
};
...
static void *do_slabs_alloc(const size_t size, unsigned int id, uint64_t *total_bytes,
    ...
    if (p->sl_curr != 0) {
        ...
        if (it->next) {
            it->next->prev = 0;
#ifdef PSLAB
            pmem_member_persist(it->next,prev);
#endif
        }
...
void do_slabs_free(void *ptr, const size_t size, unsigned int id) {
    ...
    if ((it->it_flags & ITEM_CHUNKED) == 0) {
        ...
        it->slabs_clsid = 0;////
#ifdef PSLAB
        pmem_member_persist(it,slabs_clsid);
#endif
        it->prev = 0;///
#ifdef PSLAB
        pmem_member_persist(it,prev);
#endif
        it->next = p->slots;////
#ifdef PSLAB
        pmem_member_persist(it,next);
#endif
        if (it->next) {
            it->next->prev = it;///
#ifdef PSLAB
